// public_protocols.js
// Load and display the public Hato Lab protocol list from public_protocols.csv.
fetch('public_protocols.csv')
  .then(response => {
  ...
  .catch(error => {
    console.error('Error loading public protocol data:', error);
    const status = document.getElementById('protocolStatus');
    if (status) {
      status.textContent = 'Unable to load the public protocol list. Please try again later.';
    }
    const protocolList = document.getElementById('protocolList');
